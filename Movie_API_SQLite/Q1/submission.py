    ...
    def max_degree_nodes(self) -> dict: #Complete
        ...
        for i in self.edges: 
            ...
            if i[1] not in degree_dict.keys():
                ...
                degree_dict[i[1]]+=1
        ans={}
        max_value= max(degree_dict.values())
        
        for i, j in degree_dict.items():
            if j==max_value:  
                ans[i]=j
        return ans
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list: #Complete
        ...
        if r1.status == 200:
            ...
            for i in range(len(text['cast'])):  
                #print(exclude_ids)
                #-> If exclude list is None, append list according to limit
                if exclude_ids==None:
                    if text['cast'][i]['order']<limit:
                        cast.append({
                            'id':text['cast'][i]['id'],
                            'name':text['cast'][i]['name'],
                            'character': text['cast'][i]['character'],
                            'credit_id': text['cast'][i]['credit_id'],
                            'order': text['cast'][i]['order']
                        })
                elif str(text['cast'][i]['id']) in exclude_ids: 
                    #print(True)
                    continue
                #Then continue with the list appending
                else:  
                    if text['cast'][i]['order']<limit:
                        cast.append({
                            'id':text['cast'][i]['id'],
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list: #Complete
        ...
        if r1.status == 200: 
            ...
            movie = []
            for i in range(len(text['cast'])):
                # If the movie does'nt hve the vote-average key
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return "https://poloclub.github.io/argo-graph-lite/#bd336501-7bb0-42bf-991a-ecee368329a6"
