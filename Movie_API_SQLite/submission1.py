...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None: #Complete
        """
        add a tuple (id, name) representing a node to self.nodes if it does not already exist
        The graph should not contain any duplicate nodes
        """
        temp=(str(id), str(name))
        if temp in self.nodes: #Check if node is duplicate
            #print(temp, " -> Is a duplicate node")
            return 0
        else: 
            return self.nodes.append(temp) #Add node if node is not duplicate


    def add_edge(self, source: str, target: str) -> None: #Complete
        """
    ...
    def total_nodes(self) -> int: #Complete
        """
    ...
    def total_edges(self) -> int:#Complete
        """
    ...
    def max_degree_nodes(self) -> dict: #Complete
        """
        ...
        """
        degree_dict={}
        for i in self.edges: #To count the degrees of every node

            if i[0] not in degree_dict.keys():
                degree_dict[i[0]]=1
            else:
                degree_dict[i[0]]+=1
            
            if i[1] not in degree_dict.keys():
                degree_dict[i[1]]=1
            else:
                degree_dict[i[1]]+=1
        ans={} #Store answer here
        #print(degree_dict)
        max_value= max(degree_dict.values()) #To get the maximum degree of the nodes
        #print(max_value)
        
        for i, j in degree_dict.items():  #If there are multiple maximums, write them to a dict
            if j==max_value:  
                ans[i]=j
        return ans


    def print_nodes(self): #Complete
        """
        No further implementation required
        May be used for de-bugging if necessary
        """
        print("Current listed nodes:\n",self.nodes)


    def print_edges(self): #Complete
        """
        No further implementation required
        May be used for de-bugging if necessary
        """
        print("Current listed edges:\n",self.edges)
    ...
    def write_edges_file(self, path="edges.csv")->None:
        ...
        edges_file.close()
        print("\nfinished writing edges to csv") #-> added \n for debugging
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list: #Complete
        """
        ...
        conn= http.client.HTTPConnection('api.themoviedb.org') #-> Api connection
        conn.request("GET","/3/movie/"+movie_id+"/credits?api_key="+self.api_key+"&language=en-US")
        r1 = conn.getresponse()
        #print(r1.status, r1.reason)
        cast=[]
        if limit ==None:
            limit=1000000
        if r1.status == 200: #-> Check if http is connected
            #print("Movie cast API connected!")
            array = r1.read()
            text=json.loads(array)
            
            
            for i in range(len(text['cast'])):  
                #print(exclude_ids)
                if exclude_ids==None: #-> If exclude list is None, append list according to limit
                    if text['cast'][i]['order']<limit:
                        cast.append({
                            'id':text['cast'][i]['id'],
                            'name':text['cast'][i]['name'], #-> Not sure if this will work for gradescope, added this line for graphing
                            'character': text['cast'][i]['character'],
                            'credit_id': text['cast'][i]['credit_id'],
                            'order': text['cast'][i]['order'] #-> Temporary shown for debugging
                        })
                elif str(text['cast'][i]['id']) in exclude_ids: #-> If there is something in exclude list, exclude that
                    #print(True)
                    continue
                else:  #Then continue with the list appending
                    if text['cast'][i]['order']<limit:
                        cast.append({
                            'id':text['cast'][i]['id'],
                            'name':text['cast'][i]['name'], #-> Not sure if this will work for gradescope, added this line for graphing
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list: #Complete
        """
        ...
        #print(r1.status, r1.reason)
        if r1.status == 200: #-> Check if http is connected
            print("Credit for Person API connected!")
            array = r1.read()
            text=json.loads(array)
            # print(text)
            ...
            movie = []
            for i in range(len(text['cast'])):
                if 'vote_average' in text['cast'][i]:
                    print("here")
                    if vote_avg_threshold != None: #If there is threshold, append the data according to the threshold
                        if text['cast'][i]['vote_average'] >= vote_avg_threshold: 
                            movie.append({
                                'id': text['cast'][i]['id'],
                                'title': text['cast'][i]['title'],
                                'vote_avg': text['cast'][i]['vote_average']
                                })
                    else: #-> If no threshold, append the data directly
                        movie.append({
                            'id': text['cast'][i]['id'],
                            'title': text['cast'][i]['title'],
                            'vote_avg': text['cast'][i]['vote_average']
            ...
            if len(movie)==0: #-> If no movie applies to your provided threshold, raise this warning
                print("This actor/actress has no movies that can surpass your threshold of "+str(vote_avg_threshold))
...
def return_name()->str: #Have to double check on this
    """
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return "https://poloclub.github.io/argo-graph-lite/#191ac17d-8496-4917-8703-d2f588b8a3b0"
    #return "https://poloclub.github.io/argo-graph-lite/#ea4f719d-3896-4140-84eb-a15c365ace2f"#
...
if __name__ == "__main__":
    ...
    # graph.add_node(id='6969', name='TSTSTS')
    # graph.add_node(id='7979', name='CCCCCC')
    # graph.add_edge('2975','6969')
    # graph.add_edge('7979','2975')
    # graph.add_edge('2975','7979') #-> For testing (duplicate)
    # graph.add_edge('2975','7979') #-> For testing (duplicate)
    # #graph.add_edge('7979','69688') #-> For testing (item that doesn't exist)
    # graph.add_edge('7979','6969')
    ...
    # graph.write_nodes_file()
    # '''
    # print("Total number of edges: ",graph.total_edges())
    # print("Total number of nodes: ",graph.total_nodes())
    # print("The nodes that have the maximum degree: ",graph.max_degree_nodes())
    
    # print("\n--For Q1.1b -- \n")
    tmdb_api_utils = TMDBAPIUtils(api_key='b8ba0504529e0adbcd93487377d9dc65')
    # print(tmdb_api_utils.get_movie_cast('329', limit=5, exclude_ids=[4783]))
    # print(tmdb_api_utils.get_movie_credits_for_person(person_id='287', vote_avg_threshold=8.5))

    # call functions or place code here to build graph (graph building code not graded)
    # Suggestion: code should contain steps outlined above in BUILD CO-ACTOR NETWORK

    ##############Time to build some graphs################3
    ...
    Base_Graph=Graph()
    Act_list=[]
    Base_Graph_API_call=TMDBAPIUtils(api_key='b8ba0504529e0adbcd93487377d9dc65')
    Base_Graph.add_node(id=Base_node,name='Laurence Fishburne')
    print('success2')
    Movies=Base_Graph_API_call.get_movie_credits_for_person(person_id=Base_node,vote_avg_threshold=8.0)
    print('success')
    for i in Movies: #-> Constructing base graph
        temp=tmdb_api_utils.get_movie_cast(str(i['id']), limit=3,exclude_ids=[Base_node])
        for i in temp:
            Act_list.append(str(i['id']))
            Base_Graph.add_node(id=str(i['id']), name=str(i['name']))
            Base_Graph.add_edge(source=Base_node, target=str(i['id']))
    # print(Act_list)
    
    Act_list_2=[]
    for k in Act_list: #-> First iteration after base graph
        Movies=Base_Graph_API_call.get_movie_credits_for_person(person_id=k,vote_avg_threshold=8.0)
        for p in Movies:
            temp=tmdb_api_utils.get_movie_cast(str(p['id']), limit=3)
            for h in temp:
                if h['id'] not in Act_list_2:
                    Act_list_2.append(str(h['id']))
                Base_Graph.add_node(id=str(h['id']), name=str(h['name']))
                Base_Graph.add_edge(source=str(k), target=str(h['id']))
                
    for k in Act_list_2: #-> Second iteration
        Movies=Base_Graph_API_call.get_movie_credits_for_person(person_id=k,vote_avg_threshold=8.0)
        for p in Movies:
            temp=tmdb_api_utils.get_movie_cast(str(p['id']), limit=3)
            for h in temp:
    ...
    Base_Graph.print_nodes()
    Base_Graph.print_edges()
    Base_Graph.write_edges_file()
    Base_Graph.write_nodes_file()
