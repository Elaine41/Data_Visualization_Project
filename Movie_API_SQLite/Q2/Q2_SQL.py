...
class HW2_sql():
    ...
    def part_aiii(self,connection):
        ...
        part_aiii_sql = "CREATE TABLE cast_bio (cast_id i, cast_name t, birthday t, popularity r);"
        ######################################################################
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_c_sql = '''SELECT printf("%.2f",a.num*100.0/b.total) AS proportion 
                        FROM 
                        (SELECT COUNT(id) AS num FROM movies WHERE score>50 AND title LIKE "%war%") a,
                        (SELECT COUNT(*) AS total FROM movies) b ;
        '''
        ######################################################################
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = "SELECT cast_name, count(1) FROM movie_cast WHERE popularity >= '10.0' GROUP BY cast_name ORDER BY count(1) DESC LIMIT 5"
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = '''SELECT title AS movie_title, printf("%.2f",score) AS movie_score, COUNT(cast_id) AS cast_count
    ...
    def part_g(self,connection):
        ...
        part_g_sql ='''CREATE VIEW good_collaboration AS
                        SELECT mv1.cast_id AS cast_member_id1, mv2.cast_id AS cast_member_id2, COUNT(mv1.movie_id) AS movie_count, AVG(score) AS average_movie_score
                        FROM movie_cast mv1
                        INNER  JOIN movie_cast mv2 ON (mv1.movie_id = mv2.movie_id AND mv1.cast_id < mv2.cast_id)
                        INNER JOIN movies m ON mv1.movie_id=m.id
                        GROUP BY mv1.cast_id, mv2.cast_id
    ...
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = '''SELECT g_c.cast_member_id1 AS cast_id,  m_c.cast_name AS cast_name, printf(\"%.2f\",AVG(g_c.average_movie_score)) AS collaboration_score
                            FROM good_collaboration g_c
                            INNER JOIN movie_cast m_c ON g_c.cast_member_id1 = m_c.cast_id
                            GROUP BY cast_id
    ...
    def part_h(self,connection,path):
        ############### EDIT SQL STATEMENT ###################################
        part_h_sql = "CREATE VIRTUAL TABLE movie_overview USING fts4(id i, overview t);"
        ######################################################################
    ...
    def part_hi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hi_sql = "SELECT count() FROM movie_overview WHERE overview MATCH 'FIGHT OR fight OR fight.';"
        ######################################################################
    ...
    def part_hii(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hii_sql ='''SELECT count(*)
                            FROM movie_overview 
                            WHERE overview MATCH 'space NEAR/5 program OR Space NEAR/5 program'; 
        '''
        ######################################################################
